...
「一括書き換え」ではなく、**Strangler Fig パターン（締め殺しパターン）** で段階的に置き換える。

WebForms を動かしたまま一部機能を API に切り替え、最終的に WebForms を廃止する。
各フェーズは独立して検証可能な単位とし、どのフェーズでもシステムは稼働し続ける。

WinForms 移行との本質的な違い: WebForms は「動いている」がゆえに放置されやすい。
本移行は「壊れていない理由では移行しない根拠にならない」ことを設計で示す。

### .NET Framework 制約を踏まえた現実的な移行ルート

実案件では WebForms（.NET Framework）を即座に廃止できないケースが多い。
現実的には以下のルートを辿る：

| ステップ | 内容 |
|---|---|
| 1 | WebForms を動かしたまま、打刻処理だけ .NET 8 API に切り替える |
| 2 | Page_Load の集計を API 呼び出しに置き換える |
| 3 | 画面全体を React に切り替える |
| 4 | WebForms を廃止する |

本プロジェクトは Step 3 完了状態（WebForms 廃止後）をゴールとして実装する。
